...
<li>Тесты гоняются в матрице версий Python (3.10, 3.11, 3.12), чтобы убедиться, что поведение API одинаковое в разных рантаймах.</li>
<li>Используется параллельный прогон (<code>pytest -n auto</code>) и повтор неустойчивых тестов (<code>pytest-rerunfailures</code>), чтобы стабилизировать публичный стенд PetStore.</li>
<li>Артефакты в пайплайне включают Allure-репорт и junit-отчёт (для истории прогона и анализа падений).</li>
<li>Флейковые ответы PetStore (404 после создания ресурса, рассинхрон статус/данных) не ломают pipeline: такие кейсы помечаются через <code>pytest.xfail</code> с <code>allure.step</code>, чтобы в отчёте явно было видно, что это нестабильность внешнего API, а не баг тестов.</li>
</ul>
<hr>
